...
export default function App() {
  ...
  const breadcrumbSegments = useMemo(
    () =>
      currentNode && store
        ? store.breadcrumbForNode(currentNode)
        : currentMap && store
          ? store.breadcrumbForMap(currentMap)
          : [],
    [currentNode, currentMap, store],
  )
  ...
  const filteredNodes = useMemo(
    () => (currentMap ? appModel.filteredNodesIn(currentMap) : []),
    [currentMap, state.selectedLevel],
  )
  // 切换地图/级别时重置视口（对齐 Mac 版 .id() 重建语义）
  const mapKey = `${currentMap?.id ?? 'none'}-${state.selectedLevel ?? 'all'}`
