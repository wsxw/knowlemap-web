...
- **hash 路由**：视图状态进 URL——总览 `#/overview`、子系统 `#/时态系统id`、选中节点 `#/tense_system/present_simple`；刷新/分享链接/浏览器前进后退都还原位置，非法 hash 自动回退总览。默认打开即英语总览，且每次进入总览（含力学沉降后）自动适配窗口。
- **多模块 + 分级导航**：侧栏按「模块 → 子系统」分组，A1/A2 级别筛选。
- **英语总览地图**：点侧栏「🇬🇧 英语」进入全景图——「英语」根节点 + 6 大模块组成 **Obsidian 风格的力导向网状图**：节点间斥力、连线上弹簧力，自动收敛成疏密均匀的网；**按住任一节点拖动，连线相连的节点会被弹簧拽着联动**，松手回弹稳定，拖完的布局记忆到 localStorage。模块按其下节点进度着色（绿=全点亮 / 橙=进行中 / 蓝=可学 / 灰=未解锁），角标显示「已点亮/总数」，📍 高亮当前所在模块，点模块直接进入对应子系统地图。
...
- **移动端适配**：窄屏（≤1080px）自动切换单屏模式——地图全屏，点节点详情滑入（带返回），侧栏收进 ☰ 抽屉；地图支持双指捏合缩放。
- **可调分栏**：地图栏与详情栏之间的分隔条可拖动调整宽度（380–760px），双击复位，宽度记忆到 localStorage。
- **Web 版新增**：「清空进度」重置按钮、可调分栏、双主题、移动端适配。
